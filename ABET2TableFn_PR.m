function [data, ABETdata, largeRewSide, smallRewSide] = ABET2TableFn_PR(filename, dummy)
...
Headers={'Trial','PressTime', 'rewDeliveryTime', 'collectionTime', 'RewSize', 'collectTrial', 'Ratio'};
data=table(zeros(5,1),zeros(5,1),zeros(5,1),zeros(5,1),zeros(5,1),zeros(5,1),zeros(5,1));
data.Properties.VariableNames([1:7])=Headers;
...
for ii=startRow:rows
    ...
    %keep track of block
    if regexp(ABETdata{ii,4},'Progressive_Ratio')

        currRatio=ABETdata{ii,9};

        
    end
    
    data.Ratio(trial)=currRatio;

    if regexp(ABETdata{ii,4},'Touch Image Increments PR counter')
        data.PressTime(trial) = ABETdata{ii,1};
        trial = trial + 1;
    end
    ...
    if regexp(ABETdata{ii,4},'Feeder #2')
        data.rewDeliveryTime(trial-1)=ABETdata{ii,1};
        data.RewSize(trial-1) = 1.2; %size of large reward, only reward available
    end
    if regexp(ABETdata{ii,4},'reward retrieved*')
        data.collectionTime(trial-1)=ABETdata{ii,1};
        data.collectTrial(trial-1)=1;
    end
end
    %TrialStart
